...
另外提醒一句：这些数字是 M5 Pro 的。MLX 快不快，很大程度看内存带宽，M1 到 M4 各代芯片的带宽都不一样，别拿 40 tok/s 直接对号入座，以自己机器实测为准。

| 场景 | Q5_K_M + MTP | qwen3.8:27b-mlx (MLX 原生) | GSQ IQ3_S + MTP* |
|------|-------------|---------------------------|------------------|
| 代码生成 | 22.4 tok/s | **40.8 tok/s** | 16.2 tok/s |
| 开放式写作 | 13.9 tok/s | **29.0 tok/s** | ~12.6 tok/s |
...
ISTA-DASLab 的 **GSQ-RCO** 把 Qwen3.8-27B 压到 IQ3_S（≈12GB）/ IQ3_XXS（≈10GB）且基本无损。我们在本机（M5 Pro / 48GB / Ollama 0.33.3）跑了 **4 模型严格对照**：`temperature=0`、统一 12 道题（代码 / 数学 / 知识 GPQA / 长上下文 / 严格 JSON / 中文写作）、`num_ctx=131072`、KV 缓存 `q8_0`、四模型均开 MTP（`draft_num_predict 3`）、串行跑完即卸载避免显存叠加。

> 命名对应：GitHub 仓库里 MLX 版记作 **MLX nvfp4** / **MLX 原生版**，模型标签 `qwen3.8:27b-mlx`；GSQ 指 ISTA-DASLab 的 GSQ-RCO 量化。

| 模型 | 量化 | 体积 | 平均 decode（12 题） | 质量（12 题严格校验） |
|------|------|------|---------------------|----------------------|
| **MLX nvfp4**（MLX 原生版） | NVFP4 均匀 4.0bpw | 18 GB | **29.9 tok/s** | 12/12 全对 |
...
> 上方「M5 Pro 48GB 性能基准」里的 40.8 / 22.4 是早期短上下文不同测法的基准；本表为 `temperature=0`、12 题统一对照，更可控。两表都来自本机实测。

**三个维度的结论（本机 M5 Pro / 48GB / Ollama 0.33.3 实测）：**

- **更省内存 ✅**：IQ3_S 比 MLX 4-bit 省 6GB，IQ3_XXS 省 8GB——直接给 131K 上下文或同机跑的 MiniMax H3 视频模型腾地方。
- **更快 ❌**：MLX 4-bit 平均 decode 29.9 tok/s，约为 GSQ 混合精度（12~13 tok/s）的 **2.4 倍**。NVFP4 是 GPU 友好的均匀 4-bit，IQ/GSQ 混合精度每个权重解量化开销大，在 Apple Silicon 的 Metal 后端（memory-bound）反而更慢。
- **质量无损（vs MLX）✅**：12 题四档全对（JSON 严格可解析、数学最终答案一致、知识题选项一致）。DASLab 在 198 题 GPQA 上测得 IQ3_XXS 相对 BF16 差 ~1 分，需大题集才复现。
