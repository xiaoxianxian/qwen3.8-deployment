...
**根因：** Ollama 的模型 manifest 还在，但实际权重 blob（GGUF，约 20GB）被删除或损坏。Ollama 能"列"出模型（只读 manifest），却无法加载推理，chat/generate 路由统一报 not found。`<|video_pad|>` 400 是同一损坏状态的连带症状，权重恢复后不再出现。

**排查：**
```bash
ollama show xxx --modelfile          # 记下 FROM 的 sha256-xxx
ls -la ~/.ollama/models/blobs/       # 若无 sha256-* 大文件 → blob 已丢失
...
**预防：** 切勿删除 `~/models/*.gguf` 源文件或手动清 `~/.ollama/models/blobs/`（用 `ollama rm` 安全回收）。若 blobs 目录出现两个约 19.77GB 的 `sha256-*` 文件，用 `grep -rl <digest> ~/.ollama/models/manifests/` 确认未被引用后再删，只删孤儿副本。

### 6. chat 报 500 "image input is not supported" / 提示需要 mmproj

**现象：** WorkBuddy 调用就 `500 image input is not supported`，常带 `<|video_pad|>/<|image_pad|>`。

**根因（重要，曾误判过）：** Qwen3.8-27B **原生就是多模态模型**，但 unsloth 把视觉投影（mmproj）从主 GGUF 单独抽出来了。只 `ollama create` 主 GGUF 时，模型声明视觉架构却**缺 mmproj 文件**，Ollama 收不下图片就报这个错。**正确做法不是"剥掉图片"，而是装上 mmproj 让模型真正能看图。**
...
**客户端配置：** 模型已支持视觉，`~/.workbuddy/models.json` 该模型 `"supportsImages"` 保持 `true`。`url` 默认直连 Ollama `http://localhost:11434/v1`（最稳，由 Ollama 自身管理）；超长对话想自动防超时可改用智能代理 `http://localhost:11435/v1`（见下节，默认放行图片并做上下文截断），二者都支持多模态。

### 7. chat 报 400 "exceeds the available context" / 长对话慢或超时
...
```
（plist 含 RunAtLoad 登录自启 + KeepAlive 崩溃自动重启）。验证：`curl -s -o /dev/null -w "%{http_code}" http://localhost:11435/v1/models` 应返回 `200`；挂了用 `launchctl kickstart -k gui/$(id -u)/com.user.ollama-strip-proxy` 拉起。

**推荐默认：** 直连 `11434`（`supportsImages=true`）即可稳定用好多模态；只有长会话频繁触发 400 时，再启用 `11435` 代理做截断。改完彻底重启 WorkBuddy。
...
| 量化版本 | 文件大小 | 内存占用 | 生成速度 | 质量 |
|---------|---------|---------|---------|------|
| IQ1_S | 6 GB | ~12 GB | ~8 tok/s | 中 |
| Q4_K_M | 16 GB | ~20 GB | ~7 tok/s | 高 |
| Q5_K_M | 20 GB | ~25 GB | ~6 tok/s | 很高 |
| Q8_0 | 29 GB | ~35 GB | ~5 tok/s | 最高 |
